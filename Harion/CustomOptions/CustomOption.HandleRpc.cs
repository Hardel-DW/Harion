        ...
        public static bool Prefix([HarmonyArgument(0)] byte callId, [HarmonyArgument(1)] MessageReader reader) {
            if (callId == (byte) CustomRPC.SyncroCustomGameOption) {
                ...
                if (customOption == null) {
                    HarionPlugin.Logger.LogWarning($"Received option that could not be found, sha1: \"{string.Join("", sha1.Select(b => $"{b:X2}"))}\", type: {type}.");
                    return false;
                }
                ...
                if (Debug)
                    HarionPlugin.Logger.LogInfo($"\"{customOption.ID}\" type: {type}, value: {value}, current value: {customOption.Value}");

                customOption.SetValue(value, true);

                if (Debug)
                    HarionPlugin.Logger.LogInfo($"\"{customOption.ID}\", set value: {customOption.Value}");
