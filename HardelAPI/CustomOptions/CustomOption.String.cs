    ...
    public class CustomStringOption : CustomOption, IStringOption {
        ...
        public CustomStringOption(string id, string name, bool saveValue, string[] values) : base(id, name, saveValue, CustomOptionType.String, 0) {
            ...
            ValueChanged += (sender, args) => {
                if (ConfigEntry != null && GameObject is StringOption && AmongUsClient.Instance?.AmHost == true && PlayerControl.LocalPlayer)
                    ConfigEntry.Value = GetValue();
            };

            ConfigEntry = saveValue ? HardelApiPlugin.Instance.Config.Bind(PluginID, ConfigID, GetDefaultValue()) : null;
