...
using HardelAPI.CustomKeyBinds.Patch;
...
namespace HardelAPI.CustomKeyBinds.Components {
    ...
    public class ClientKeyButton {
        ...
        public static void Postfix(OptionsMenuBehaviour __instance) {
            ...
            if ((streamerMods == null || streamerMods.gameObject == null)) {
                streamerMods = CreateCustomToggle("Streamer Mode: ", HardelApiPlugin.StreamerMode.Value, Vector3.right * 1.3f, (UnityAction) streamerModeToggle, __instance);

                void streamerModeToggle() {
                    HardelApiPlugin.StreamerMode.Value = !HardelApiPlugin.StreamerMode.Value;
                    UpdateToggle(streamerMods, "Streamer Mode: ", HardelApiPlugin.StreamerMode.Value);
                }
            }
    ...
    public static class HiddenTextPatch {
        private static void Postfix(TextBoxTMP __instance) {
            bool flag = HardelApiPlugin.StreamerMode.Value && (__instance.name == "GameIdText" || __instance.name == "IpTextBox" || __instance.name == "PortTextBox");
            if (flag)
